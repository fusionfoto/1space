...
from s3_sync.proxymc_app import forward_raw_swift_req


class ProxyMCAuth(object):
    """
    ...
    def __init__(self, app, conf):
        self.app = app
        self.conf = conf
        self.logger = get_logger(conf, log_route='proxymc_auth')
    ...
    def __call__(self, env, start_response):
        ...
        storage URL are cached locally to allow rudimentary authz for proxymc
        requests into S3 (or whatever).  The returned storage URL is mutated to
        point to this proxymc server, not the Swift cluster to which we
        forwarded the auth request.
        ...
        try:
            vrs, realm, rest = split_path(env['PATH_INFO'], 2, 3, True)
        except ValueError:
            realm = 'unknown'
    ...
    def get_valid_account(self, env, token):
        ...
        :returns: None if the token is invalid or the Swift Account extracted
                  from the storage URL returned by a prior successful
                  authentication.
        """
        acct = None
        ...
        memcache_token_key = 'proxymc_auth/token/%s' % token
